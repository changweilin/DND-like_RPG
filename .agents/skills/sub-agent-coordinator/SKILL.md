...
1. **Library & API Reference**: Explain how to correctly use internal/common libraries, CLIs, or SDKs.
   - *Positive Examples*: `billing-lib` (documenting edge cases, footguns), `internal-platform-cli` (subcommands with examples), `frontend-design` (pushing Codex toward company-specific design systems).
   - *Anti-Pattern*: Stating the obvious. Don't teach Codex basic standard coding; focus on information that pushes Codex out of its normal way of thinking.
...
2. **Product Verification**: Test or verify that code is working, often paired with external tools (Playwright, tmux).
   ...
   - *Anti-Pattern*: Relying purely on Codex's visual guessing. Enforce programmatic assertions on state at each step.
...
6. **Code Quality & Review**: Enforce code quality and review code inside the org using deterministic tools.
   - *Positive Examples*: `adversarial-review` (spawning a fresh-eyes subagent to critique, iterate until nitpicks), `code-style` (enforcing styles Codex misses by default), `testing-practices` (instructions on how/what to test).
...
2. **Build a Gotchas Section**: Maintain a high-signal section for common failure points. Update this over time as Codex hits new edge cases.
3. **Progressive Disclosure**: A skill is a folder, not just a markdown file. Split details into sub-files (e.g., `references/api.md` or `assets/template.md`) and point Codex to read them when appropriate.
4. **Avoid Railroading Codex**: Describe goals and intent. Give Codex the information it needs but provide the flexibility to adapt to the situation, rather than overly rigid step-by-step constraints.
5. **Description = Trigger**: The description field is NOT a summary. It is scanned by Codex to decide "is there a skill for this request?". Be highly specific about *when* to trigger it.
6. **Think through the Setup**: Use `config.json` for user-specific settings (e.g., Slack channels). If the config isn't set, instruct Codex to use the `AskUserQuestion` tool. Don't annoy the user repeatedly.
7. **Persistent Memory**: Store execution logs or JSON data in `${CLAUDE_PLUGIN_DATA}` so Codex can read its own history (e.g., knowing what changed since yesterday) without data wiping on upgrades.
8. **Store Scripts & Generate Code**: Provide helper scripts/libraries so Codex spends its turns on composition and analysis rather than reconstructing boilerplate.
